...
import sqlite3
import Bookcar,Addmember,Add_destination
...
class Main(Toplevel):
    def __init__(self):
        ...
        def displayStatistics(evt):
            count_cars = cur.execute("SELECT count(car_id) FROM Cab_booking").fetchall()
            count_members = cur.execute("SELECT count(User_id) FROM Users").fetchall()
            booked_cars = cur.execute("SELECT count(car_status) FROM Cab_booking WHERE car_status=1").fetchall()
            print(count_cars)
            self.lbl_car_count.config(text='Total :' + str(count_cars[0][0]) + ' cars available')
            self.lbl_member_count.config(text="Total users : " + str(count_members[0][0]))
            self.lbl_taken_count.config(text="Booked Cars:" + str(booked_cars[0][0]))
            displaycars(self)
        ...
        def displaycars(self):
            ...
            def carInfo(evt):
                ...
                id=value.split('-')[0]
                car =cur.execute("SELECT * FROM Cab_booking WHERE car_id=?",(id,))
                car_info=car.fetchall()
                print(car_info)
                self.list_details.delete(0,'end')
                self.list_details.insert(0,"Car Name : "+car_info[0][1])
                self.list_details.insert(1,"Car type : "+car_info[0][2])
                self.list_details.insert(2,"Model : "+car_info[0][3])
                self.list_details.insert(3,"Car_status : "+str(car_info[0][4]))
                if car_info[0][4] == 0:
        ...
        self.welcome = Label(Topframe, text="Welcome to CAB MANAGEMENT SYSTEM of LPU", pady=20, font='sans-serif 10 ')
        self.welcome.pack(side=LEFT,padx=50)
        ...
        self.tabs.add(self.tab1, text='Cab Management', image=self.tab1_icon, compound=LEFT)
